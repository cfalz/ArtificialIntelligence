...
class UniformCostSearch(Search):
    ...
    def search(self):
        ...
        while self.container:
            ...
            if PRINTDEBUG:
                print "The Current Node Is: ", current_node , "At Depth : ", current_node.path_cost
...
class AstarMisplacedTileSearch(Search):
    ...
    def search(self):
        ...
        while self.container:
            ...
            current_node = self.container.pop(0)
            if PRINTDEBUG:
                print "The Current Node Is: \n", current_node.state , "\n At Depth : ", current_node.path_cost
            ...
            for new_node in new_nodes:
                ...
                if (self.not_in_container(new_node.state) and (self.not_in_explored(new_node.state))): 
                    if PRINTDEBUG:
                        print "Its not in the container and we have not explored this state, Appending to Container." 
                        print "\n"
                    bisect.insort(self.container,new_node)
...
class AstarManhattanDistanceSearch(Search):
    ...
    def node_insert(self, a, x, lo=0, hi=None):
        if lo < 0:
        ...
        while lo < hi:
            mid = (lo+hi)//2
            if self.get_total_cost(a[mid]) < self.get_total_cost(x): 
                lo = mid+1
            else: hi = mid
        a.insert(lo,x)
    ...
    def search(self):
        node = Node(self.problem.initial_state, 0) 
        self.node_insert(self.container,node)
        while self.container:
            current_node = self.container.pop(0)
            if PRINTDEBUG:
            ...
            for new_node in new_nodes:
                ...
                if self.not_in_container(new_node.state) and self.not_in_explored(new_node.state): 
                    if PRINTDEBUG:
                        print "Its not in the container and we have not explored this state, Appending to Container." 
                        print "\n"
                    self.node_insert(self.container,new_node)
